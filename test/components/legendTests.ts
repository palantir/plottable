...
describe("Legend", () => {
  ...
  it("a basic legend renders", () => {
    ...
    rows.each(function(d: any, i: number) {
      ...
      var symbol = d3this.select("." + Plottable.Component.Legend.LEGEND_ICON_CLASS);
  ...
  it("re-rendering the legend with a new domain will do the right thing", () => {
    ...
    (<any> legend)._content.selectAll(entrySelector).each(function(d: any, i: number) {
      ...
      var fill = d3.select(this).select("." + Plottable.Component.Legend.LEGEND_ICON_CLASS).attr("fill");
  ...
  it("scales icon sizes properly with font size (textHeight / 2 < iconHeight < textHeight)", () => {
    color.domain(["foo"]);
    legend.renderTo(svg);
    ...
    function verifyIconHeight() {
      ...
      var icon = (<any> legend)._content.select("." + Plottable.Component.Legend.LEGEND_ICON_CLASS);
