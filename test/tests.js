...
describe("Category Axes", function () {
    ...
    it("proper range values for different range types", function () {
        ...
        var ticksNormalizedPosition = ca._tickMarkContainer.selectAll(".tick-mark")[0].map(function (s) { return +d3.select(s).attr("x1") / step; });
        assert.deepEqual(ticksNormalizedPosition, [1, 2, 3]);
        xScale.rangeType("points", 1, 0);
        step = SVG_WIDTH / 4;
        ticksNormalizedPosition = ca._tickMarkContainer.selectAll(".tick-mark")[0].map(function (s) { return +d3.select(s).attr("x1") / step; });
        assert.deepEqual(ticksNormalizedPosition, [1, 2, 3]);
        svg.remove();
    });
});
