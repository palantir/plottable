...
describe("TimeAxis", function () {
    ...
    it.skip("min interval on time Axis", function () {
        var svg = generateSVG(400, 100);
        var scale = new Plottable.Scale.Time();
        var axis = new Plottable.Axis.Time(scale, "bottom");
        ...
        scale.range([0, 600]);
        axis.renderTo(svg);
        // ticks are generated by days even they could be generated by 12 hours.
    });
    it.skip("custom intervals on time Axis", function () {
        var svg = generateSVG(400, 100);
        var scale = new Plottable.Scale.Time();
        var axis = new Plottable.Axis.Time(scale, "bottom");
        var intervals = axis.axisTierIntervals();
        var onlySecondIntervals = intervals.slice(1);
        onlySecondIntervals[0].tiers[0].formatter = function (d) { return "test " + d.toString(); };
        axis.axisTierIntervals(onlySecondIntervals);
        var now = new Date();
        var fourMinutesBefore = new Date(now.getTime());
        fourMinutesBefore.setMinutes(now.getMinutes() - 4);
        scale.domain([fourMinutesBefore, now]);
        scale.range([0, 600]);
        axis.renderTo(svg);
        // ticks are generated by seconds and using custom formatter;
    });
});
