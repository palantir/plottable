...
describe("Plots", () => {
  ...
  describe("AreaPlot", () => {
    ...
    it("draws area and line correctly", () => {
      const areaPath = renderArea.select(".area");
      ...
      const areaComputedStyle = window.getComputedStyle(<Element> areaPath.node());
      ...
      const lineComputedStyle = window.getComputedStyle(<Element> linePath.node());
    ...
    it("area is appended before line", () => {
      const paths = renderArea.selectAll("path")[0];
      const areaSelection = renderArea.select(".area")[0][0];
      const lineSelection = renderArea.select(".line")[0][0];
    ...
    it("cleans up correctly when removing Datasets", () => {
      ...
      const paths = areaPlot.content().selectAll("path");
    ...
    it("correctly handles NaN and undefined x and y values", () => {
      ...
      const expectedPath = "M0,500L100,400L100,500L0,500ZM300,200L400,100L400,500L300,500Z";
      const areaPath = renderArea.select(".area");
    ...
    describe("selections()", () => {
      ...
      it("retrieves all selections with no args", () => {
        const newTwoPointData = [{ foo: 2, bar: 1 }, { foo: 3, bar: 2 }];
      ...
      it("retrieves correct selections", () => {
        const twoPointDataset = new Plottable.Dataset([{ foo: 2, bar: 1 }, { foo: 3, bar: 2 }]);
        ...
        const selectionData = allAreas.data();
      ...
      it("skips invalid Datasets", () => {
        ...
        const allAreas = areaPlot.selections([twoPointDataset, dummyDataset]);
        assert.strictEqual(allAreas.size(), 2, "areas/lines retrieved");
        const selectionData = allAreas.data();
        assert.include(selectionData, twoPointDataset.data(), "new dataset data in selection data");
