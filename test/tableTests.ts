...
describe("Tables", () => {
  ...
  it("table space fixity calculates properly", () => {
    ...
    assert.isFalse(table.fixedWidth(), "width not fixed when some subcomponent not fixed");
    assert.isTrue(table.fixedHeight(), "but the height is still fixed");
    components[8].fixedHeightVal = false;
    components[0].fixedWidthVal = true;
    assert.isTrue(table.fixedWidth(), "width fixed again");
    assert.isFalse(table.fixedHeight(), "now height unfixed");
  });
});
