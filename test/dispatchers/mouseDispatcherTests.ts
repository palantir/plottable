...
describe("Dispatchers", () => {
  describe("Mouse Dispatcher", () => {
    ...
    it("getDispatcher() creates only one Dispatcher.Mouse per <svg>", () => {
      ...
      const md1 = Plottable.Dispatchers.Mouse.getDispatcher(<SVGElement> svg.node());
      assert.isNotNull(md1, "created a new Dispatcher on an SVG");
      const md2 = Plottable.Dispatchers.Mouse.getDispatcher(<SVGElement> svg.node());
    ...
    it("lastMousePosition() defaults to a non-null value", () => {
      ...
      const md = Plottable.Dispatchers.Mouse.getDispatcher(<SVGElement> svg.node());
      const p = md.lastMousePosition();
    ...
    it("can remove callbacks by passing null", () => {
      ...
      const md = Plottable.Dispatchers.Mouse.getDispatcher(<SVGElement> target.node());

      let cb1Called = false;
      const cb1 = (p: Plottable.Point, e: MouseEvent) => cb1Called = true;
      let cb2Called = false;
      const cb2 = (p: Plottable.Point, e: MouseEvent) => cb2Called = true;
    ...
    it("doesn't call callbacks if not in the DOM", () => {
      ...
      const callback = (p: Plottable.Point, e: MouseEvent) => callbackWasCalled = true;
    ...
    it("doesn't call callbacks for clicks if obscured by overlay", () => {
      ...
      const overlay = TestMethods.getSVGParent().append("div")
            .style({
              height: "400px",
              width: "400px",
              position: "absolute",
    ...
    it("calls callbacks on mouseover, mousemove, and mouseout", () => {
      ...
      let callbackWasCalled = false;
      const callback = (p: Plottable.Point, e: MouseEvent) => {
    ...
    it("onWheel()", () => {
      ...
      const targetWidth = 400, targetHeight = 400;
      const svg = TestMethods.generateSVG(targetWidth, targetHeight);
      // HACKHACK: PhantomJS can't measure SVGs unless they have something in them occupying space
      ...
      let callbackWasCalled = false;
      const callback = (p: Plottable.Point, e: WheelEvent) => {
        callbackWasCalled = true;
