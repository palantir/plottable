...
describe("Word Wrap Utilities", () => {
    it("properly wraps a short sentence", () => {
        ...
        var width = 200;
        var wrappedText = Plottable.WordWrapUtils.breakTextToFitWidth(text, width, (text) => text.length * 10);
    ...
    it("breaks on line break characters in the correct place", () => {
        ...
        text = "||||| d[d";
        wrappedText = Plottable.WordWrapUtils.breakTextToFitWidth(text, width, (text) => text.length * 10);
    ...
    it("can predict when it will and won't break", () => {
        ...
        var canBreak = (text: string) => Plottable.WordWrapUtils.canWrapWithoutBreakingWords(text, 70, measureText);
