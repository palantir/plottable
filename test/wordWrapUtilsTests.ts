...
describe("Word Wrap Utilities", () => {
    it("properly wraps a short sentence", () => {
        var text = "All work and no play makes Jack a dull boy.";
        var width = 200;
        var wrappedText = Plottable.Utils.WordWrap.breakTextToFitWidth(text, width, (text) => text.length * 10);
    ...
    it("breaks on line break characters in the correct place", () => {
        ...
        text = "||||| d[d";
        wrappedText = Plottable.Utils.WordWrap.breakTextToFitWidth(text, width, (text) => text.length * 10);
    ...
    it("can predict when it will and won't break", () => {
        ...
        var canBreak = (text: string) => Plottable.Utils.WordWrap.canWrapWithoutBreakingWords(text, 70, measureText);
