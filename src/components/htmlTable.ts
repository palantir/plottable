...
export class HTMLTable extends HTMLComponentContainer {
  ...
  private _iterateLayout(availableWidth: number, availableHeight: number, isFinalOffer = false): _IterateLayoutResult {
    ...
    let rowWeights = HTMLTable._calcComponentWeights(this._rowWeights, rows, (c: IComponent<any>) => (c == null) || c.fixedHeight());
    let colWeights = HTMLTable._calcComponentWeights(this._columnWeights, cols, (c: IComponent<any>) => (c == null) || c.fixedWidth());
    ...
    let colProportionalSpace = HTMLTable._calcProportionalSpace(heuristicColWeights, availableWidthAfterPadding);
    let rowProportionalSpace = HTMLTable._calcProportionalSpace(heuristicRowWeights, availableHeightAfterPadding);
  ...
  public fixedWidth(): boolean {
    let cols = d3.transpose(this._rows);
    return HTMLTable._fixedSpace(cols, (c: IComponent<any>) => (c == null) || c.fixedWidth());
  }

  public fixedHeight(): boolean {
    return HTMLTable._fixedSpace(this._rows, (c: IComponent<any>) => (c == null) || c.fixedHeight());
  }
