...
export module _Drawer {
  export class Line extends AbstractDrawer {
    ...
    public _getDatum(selection: D3.Selection, pixelPoint: Point): any {
      var datum: any;
      selection.data().forEach((selectionDatum: any, index: number) => {
        if (this._xFunction(selectionDatum, index) === pixelPoint.x && this._yFunction(selectionDatum, index) === pixelPoint.y) {
