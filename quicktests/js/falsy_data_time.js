...
function run(div, data, Plottable) {
  ...
  var scale_array = [];
  for (var i = 0; i < 100; i++){
      scale_array.push(new Plottable.Scale.Time());
      scale_array.push(new Plottable.Scale.Linear());
  }

  var axis_array = [];
  for (var i = 0; i < 100; i += 2){
      axis_array.push(new Plottable.Axis.Time(scale_array[i], "bottom"));
      axis_array.push(new Plottable.Axis.Numeric(scale_array[i+1], "left"));
  }

  //Labels
  var title = new Plottable.Component.TitleLabel("null , NaN, undefined, false, \"\" on Time with y=1");
