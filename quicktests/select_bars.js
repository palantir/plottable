...
function run(div, data, Plottable) {
  ...
  cb_drag = function(xy) {
      ...
      plot1.selectBar({min: xy.xMin, max: xy.xMax},
  ...
  var drag_interaction = new Plottable.Interaction.XYDragBox(renderGroup)
                                      .dragend(cb_drag)
                                      .registerWithComponent();

  var click_interaction = new Plottable.Interaction.Click(renderGroup)
                                      .callback(cb_click)
                                      .registerWithComponent();

  var reset_interaction = new Plottable.Interaction.Click(title)
                                      .callback(cb_reset)
                                      .registerWithComponent();
}
