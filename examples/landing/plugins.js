function pluginsChart(gitData) {
  var xScale = new Plottable.Scales.Linear().domain([-0.03, 1.03]);
  var yScale = new Plottable.Scales.Log().domain([0.9, 100000]);
  var colorScale = new Plottable.Scales.Color("category10");
  ...
  var renderer  = new Plottable.Plots.Scatter(gitData, xScale, yScale)
  ...
  var legend    = new Plottable.Components.Legend(colorScale).xOffset(-80);
  var center = renderer.merge(gridlines).merge(legend);

  if (!window.mobilecheck()) {
    var dragBox = new Plottable.Interactions.XYDragBox(center).registerWithComponent().setupZoomCallback(xScale, yScale);
  };
  ...
  new Plottable.Templates.StandardChart().xAxis(xAxis).yAxis(yAxis).center(center)
